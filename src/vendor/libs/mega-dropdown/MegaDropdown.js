...
class MegaDropdown extends Component {
  ...
  createTimeout(ignoreFocus = false) {
    this.resetTimeout();
    this.timeoutId = setTimeout(() => this.close(false, ignoreFocus), this.options.timeout);
  }
  ...
  onClick(e) {
    ...
    if (e.target === this.toggle && !this.options.openOnHover) {
      ...
      e.preventDefault();
      this.open();
    } else if (this.isMegaLink(e.target)) this.close(true);
  ...
  render() {
    return <this.props.as {...this.elOptions} show={this.state.show} ref={(el) => (this._el = el)} />;
  }
}
